...
- [dev-done] **Rename `IngestLagTest` to Match What It Proves:** The test hard-codes `eventTime = now - 50` so it only validates gauge math, not actual system lag. Renamed to `LagGaugeTest`. *(Claude)*
